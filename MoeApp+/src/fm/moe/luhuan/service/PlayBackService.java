...
public class PlayBackService extends Service{
	...
	private void playSongAtIndex(int n){
		...
		sendNotification(R.drawable.ic_media_play, item.getTitle(), "ÕýÔÚ²¥·Å", item.getTitle());
		isPrepared = false;
		String url = "";
		try{
	...
	private void sendNotification(int drawableId, String tickerText,
			...
			if(tickerText!=null){
				nBuilder.setTicker("ÃÈ·ñÒôÀÖ:" + tickerText);	
			}
			
			nBuilder.setContentTitle("ÃÈ·ñÒôÀÖ:" + title);
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getIntExtra("state", 1) == 0) {
				if(mPlayer.isPlaying()){
					mPlayer.pause();
					sendNotification(R.drawable.ic_media_pause, "²¥·ÅÒÑÔÝÍ£",
							"²¥·ÅÒÑÔÝÍ£", "¶ú»úÒÑ°Î³ö");
				}
			}
